...
void solve() {
    ...
    function<ll(int, int, int)> dp = [&](int i, int j, int k) -> ll {
        // If we have no cards to choose from
        if (i == 0) {
            // If the sum is satisifed and we have already chosen k cards
            return (j == 0 && k == 0) ? 1 : 0;
        }
        ...
        // We don't pick the current card
        ll out = dp(i - 1, j, k);

        // If the sum is greater than or equal to zero
        if (j - arr[i - 1] >= 0) {
            out += dp(i - 1, j - arr[i - 1], k - 1);
        }
    ...
    // For each sum j, we will check which combination of numbers will give us
    // an average of A
    FOR(j, 1, N * 50 + 1) {
        FOR(k, 1, N + 1) {
            if (static_cast<double>(j) / k == A) {
                res += dp(N, j, k);
            }
        }
    }
